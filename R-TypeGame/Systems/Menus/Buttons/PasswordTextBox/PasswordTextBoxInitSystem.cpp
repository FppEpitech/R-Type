...
#include "PasswordTextBoxInitSystem.hpp"
#include "ButtonStateComponent.hpp"
#include "DefaultTextComponent.hpp"

PasswordTextBoxInitSystem::PasswordTextBoxInitSystem() :
...
static void handleThis(ECS::Registry& reg, int idxPacketEntities)
{
    ...
    if (state && text && defaultText) {
        if (state->state == ButtonStateComponent::ButtonState::NONE) {
            if (text->text.empty())
...
void PasswordTextBoxInitSystem::_initButton(ECS::Registry& reg, int idxPacketEntities)
{
...
extern "C" {
    EXPORT_SYMBOL ISystem* loadSystemInstance() {
        return new PasswordTextBoxInitSystem();
    }
}
