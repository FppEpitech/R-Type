...
bool Users::userIdExists(int id) {
    ...
    if (!stmt)
        return false;
    sqlite3_bind_text(stmt.get(), 1, std::to_string(id).c_str(), -1, SQLITE_STATIC);
...
std::string Users::getUsernameById(int id) {
    ...
    *_rc = sqlite3_step(stmt.get());
    if (*_rc == SQLITE_ROW) {
        std::string username((const char*)sqlite3_column_text(stmt.get(), 0));
        return username;
    } else {
        return std::string("");
    }
...
Users::userSettings Users::getUserSettings(int id) {
    ...
    *_rc = sqlite3_step(stmt.get());
    if (*_rc == SQLITE_ROW) {
        int res_width = sqlite3_column_int(stmt.get(), 0);
        int res_height = sqlite3_column_int(stmt.get(), 1);
        float daltonian_mode = sqlite3_column_double(stmt.get(), 2);
        std::string skin((char *)sqlite3_column_text(stmt.get(), 3));
...
bool Users::setUserSettings(int id, int res_width, int res_height, float daltonian_mode, std::string skin) {
    ...
    if (!stmt)
        return false;
    sqlite3_bind_int(stmt.get(), 1, res_width);
    sqlite3_bind_int(stmt.get(), 2, res_height);
    sqlite3_bind_double(stmt.get(), 3, daltonian_mode);
    sqlite3_bind_int(stmt.get(), 4, id);
    sqlite3_bind_text(stmt.get(), 5, skin.c_str(), -1, SQLITE_STATIC);
    *_rc = sqlite3_step(stmt.get());
    if (*_rc != SQLITE_DONE) {
...
bool Users::createUsersTable() {
    std::string reqUsers("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, password TEXT NOT NULL, daltonian_mode FLOAT, res_width INT, res_height INT, skin TEXT);");
