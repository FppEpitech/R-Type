...
bool SceneManager::ASceneManager::processInput(KEY_MAP key, Network::UDPPacket packet)
{
    if (_keysSystems[CURRENT].find(key) != _keysSystems[CURRENT].end()) {
        _keysSystems[CURRENT].at(key)->getFunction()(_registries->at(CURRENT), packet);
        return true;
    }
